...
class User(Base):
    ...
    notifications: Mapped[list['GroupNotification']] = relationship(back_populates="for_user")
...
class Group(Base):
    ...
    base_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    notifications: Mapped[list['GroupNotification']] = relationship(back_populates="group", cascade="all,delete")
...
class GroupNotification(Base):
    ...
    for_user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.telegram_id"))
