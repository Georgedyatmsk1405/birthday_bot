...
async def get_admin_groups(request: Request):
    data_page = {
        "request": request,
        "access": False,
        "title_h1": "My Administered Groups",
    }
...
async def get_group_users(request: Request, user_id: int = None, group_id: int = None):
    data_page = {"request": request, "access": False, "title_h1": "Users in Group"}
    return templates.TemplateResponse("group_users.html", data_page)


@router.get("/custom_notifications", response_class=HTMLResponse)
async def get_custom_notifications(request: Request):
    data_page = {
        "request": request,
        "access": False,
        "title_h1": "Custom Notifications",
    }
